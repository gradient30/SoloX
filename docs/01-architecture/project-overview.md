...
- **🔓 核心监控免 Root/越狱**: 性能采集无需 Android Root 或 iOS 越狱
- **📊 全面监控**: 支持 CPU、内存、网络、FPS、电池、GPU 等多维度性能指标
- **📶 Android 弱网**: QAS Network Agent 支持非 Root、按 App 生效的弱网模拟；Root `tc netem` 作为兼容方案保留
- **📈 实时分析**: 提供美观的实时数据可视化界面
...
- **内存分析**: 内存使用量和详细分析
- **网络统计**: 上行/下行流量监控
- **FPS 检测**: 帧率和卡顿分析 (支持游戏引擎: Unity/UE4/UE5/Cocos/Laya)
- **电池监控**: 电量、温度、电流、电压、功耗
- **GPU 分析**: GPU 使用率监控
- **磁盘 I/O**: 磁盘读写性能
- **温度监控**: 设备温度传感器数据
- **弱网测试**: Android Agent 非 Root 模式、Root `tc netem` 模式、网络质量探测
...
| 内存使用 | ✅ | ✅ | MB | 内存占用和详细分析 |
| 网络流量 | ✅ | ✅ | KB | 上行/下行流量统计 |
| FPS 帧率 | ✅ | ✅ | Hz | 界面渲染帧率 (含游戏引擎) |
| 电池信息 | ✅ | ✅ | 多单位 | 电量、温度、功耗等 |
| GPU 使用率 | ✅ | ❌ | % | GPU 占用率 |
| 磁盘 I/O | ✅ | ❌ | MB/s | 磁盘读写速度 |
| 设备温度 | ✅ | ❌ | °C | 设备温度传感器 |
| 弱网模拟 | ✅ | 外部工具 | 多单位 | Android 支持 Agent / Root tc / Probe；iOS 使用外部 Network Link Conditioner |
